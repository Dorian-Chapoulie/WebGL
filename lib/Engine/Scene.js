...
export class Scene {
    ...
    deleteEntity(id) {
        ...
        if (modelToDelete.rigidBody) {
            this.engine.world.removeRigidBody(modelToDelete.rigidBody)
        }
        ...
        const triggerToDelete = this.triggers.find((trigger) => trigger.id === id);
        if (triggerToDelete) {
            this.engine.world.removeRigidBody(triggerToDelete.rigidBody)
        }
    ...
    addTrigger = (id, position, size, onTrigger) => {
        const trigger = new Trigger(this.engine.world, id, position, size, onTrigger);
