...
export class Scene {
    ...
    initScene = async () => {
        ...
        this.gltfModel = new GltfModel(this.engine.gl, this.engine.skinnedProgramInfo);
        this.models.push(this.gltfModel);
        try {
            await this.gltfModel.load('/models/women/Untitled.gltf');
    ...
    //same refactor
    draw = (deltaTime) => {
        if (this.gltfModel && this.gltfModel.isLoaded) {
            this.gltfModel.update(deltaTime);
            // const currentIndex = this.gltfModel.animator.animations.findIndex(anim => anim.name === this.gltfModel.animator.currentAnimation.name);
            // if (currentIndex !== animation) {
            //    this.gltfModel.animator.play(animation);
            // }
        }
        ...
        if (this.gltfModel && this.gltfModel.isLoaded) {
            ...
            this.rotation += 1;
            this.gltfModel.draw(this.engine.gl, this.engine.skinnedProgramInfo, this.engine.viewMatrix, this.engine.projectionMatrix, [1, 0, -5], [0.1, 0.1, 0.1]);
        }
