...
export class Engine {
    ...
    startAutoSave = () => {
        ...
        this.intervalAutoSave = setInterval(() => {
            if (this.scene) {
               ...
               // console.log('Scene saved to local storage.', sceneData);
            }
        }, SAVE_INTERVAL_MS);
    }
    ...
    initShaders = () => {
        ...
        const getPointLightsUniformsLocations = () => {
            ...
            for (let i = 0; i < MAX_POINT_LIGHTS; i++) {
                ...
                ret[`pointLights${i}uadratic`] = this.gl.getUniformLocation(this.skinnedShaderProgram, `pointLights[${i}].quadratic`);
            }
            return ret;
        }
    ...
    drawDebugEngine = () => {
        ...
        for (let i = 0; i < vertices.length / 6; i += 1) {
            ...
            // Extraire la couleur (prendre la couleur du premier point)
            const color = [
                colors[i * 8],        // r
                colors[i * 8 + 1],    // g
                colors[i * 8 + 2],    // b
