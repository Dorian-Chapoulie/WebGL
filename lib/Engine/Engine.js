...
export class Engine {
    ...
    drawDebugLines = (selectedEntity) => {
        ...
        const { vertices: selectedVertices } = this.world.debugRender(undefined, (collider) => {
            return collider.userData?.id == selectedEntity.id;
        });
        ...
        for (let i = 0; i < selectedVertices.length / 6; i += 1) {
            // Extraire les deux points de la ligne (Rapier utilise des paires de selectedVertices)
            const startPoint = [
                selectedVertices[i * 6],      // x1
                selectedVertices[i * 6 + 1],  // y1
                selectedVertices[i * 6 + 2]   // z1
            ];

            const endPoint = [
                selectedVertices[i * 6 + 3],  // x2
                selectedVertices[i * 6 + 4],  // y2
                selectedVertices[i * 6 + 5]   // z2
            ];
