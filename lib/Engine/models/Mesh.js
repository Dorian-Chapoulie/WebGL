...
import { AbstractEntity } from "./interfaces/interfaces";

export class Mesh extends AbstractEntity {
    ...
    async load(gltfPath) {
        try {
            ...
            // Extraire l'échelle du premier nœud racine si elle existe
            if (gltf.scenes && gltf.scenes.length > 0) {
                const scene = gltf.scenes[gltf.scene || 0];
                if (scene.nodes && scene.nodes.length > 0) {
                    const rootNode = gltf.nodes[scene.nodes[0]];
                    if (rootNode && rootNode.scale) {
                        // Initialiser this.scale s'il n'existe pas déjà (sera surchargé par Model)
                        if (!this.modelScale) {
                            this.modelScale = { x: 1, y: 1, z: 1 };
                        }
                        // Multiplier l'échelle du fichier GLTF avec l'échelle existante
                        this.modelScale.x *= rootNode.scale[0];
                        this.modelScale.y *= rootNode.scale[1];
                        this.modelScale.z *= rootNode.scale[2];
                    }
                }
            }
    ...
    draw(viewMatrix, projectionMatrix, position, scale, rotation) {
        ...
        // Vérifier si rotation est un quaternion (a une propriété 'w') ou des angles Euler
        if (rotation.w !== undefined) {
            // C'est un quaternion, utiliser fromRotationTranslationScale
            const rotationMatrix = mat4.create();
            mat4.fromQuat(rotationMatrix, [rotation.x, rotation.y, rotation.z, rotation.w]);
            mat4.multiply(this.modelMatrix, this.modelMatrix, rotationMatrix);
        } else {
            // C'est des angles Euler, utiliser l'ancien comportement
            ...
            if (mesh && mesh.boundingBox) {
                return {
                    ...
                    boundingBox: {
                        ...
                        halfExtents: {
                            x: (mesh.boundingBox.width * scaleX) / 2,
                            y: (mesh.boundingBox.height * scaleY) / 2,
                            z: (mesh.boundingBox.depth * scaleZ) / 2
                        }
                    }
                };
            }
    ...
    getOverallDimensions() {
        if (this.meshes.length === 0 || !this.gltfData) {
            return null;
        }

        // Échelle du Model (définie par l'utilisateur)
        const modelScaleX = this.scale?.x ?? 1;
        const modelScaleY = this.scale?.y ?? 1;
        const modelScaleZ = this.scale?.z ?? 1;
        ...
        // Fonction récursive pour parcourir la hiérarchie de nœuds
        const processNode = (nodeIndex, parentMatrix) => {
            const node = this.gltfData.nodes[nodeIndex];
            if (!node) return;

            // Construire la matrice de transformation du nœud
            let nodeMatrix = mat4.create();
            if (node.matrix) {
                mat4.copy(nodeMatrix, node.matrix);
            } else {
            ...
            // Combiner avec la matrice parente
            const worldMatrix = mat4.create();
            mat4.multiply(worldMatrix, parentMatrix, nodeMatrix);

            // Si ce nœud a un mesh, transformer sa bounding box
            if (node.mesh !== undefined) {
                const meshData = this.meshes.find(m => m.meshIndex === node.mesh);
                if (meshData && meshData.boundingBox) {
                    const bb = meshData.boundingBox;

                    // Transformer les 8 coins de la bounding box
                    const corners = [
                        [bb.min.x, bb.min.y, bb.min.z, 1],
                        [bb.max.x, bb.min.y, bb.min.z, 1],
                        [bb.min.x, bb.max.y, bb.min.z, 1],
                        [bb.max.x, bb.max.y, bb.min.z, 1],
                        [bb.min.x, bb.min.y, bb.max.z, 1],
                        [bb.max.x, bb.min.y, bb.max.z, 1],
                        [bb.min.x, bb.max.y, bb.max.z, 1],
                        [bb.max.x, bb.max.y, bb.max.z, 1]
                    ];

                    corners.forEach(corner => {
                        const transformed = [0, 0, 0, 0];
                        // Multiplier la matrice monde avec le coin
                        transformed[0] = worldMatrix[0] * corner[0] + worldMatrix[4] * corner[1] + worldMatrix[8] * corner[2] + worldMatrix[12] * corner[3];
                        transformed[1] = worldMatrix[1] * corner[0] + worldMatrix[5] * corner[1] + worldMatrix[9] * corner[2] + worldMatrix[13] * corner[3];
                        transformed[2] = worldMatrix[2] * corner[0] + worldMatrix[6] * corner[1] + worldMatrix[10] * corner[2] + worldMatrix[14] * corner[3];

                        globalMinX = Math.min(globalMinX, transformed[0]);
                        globalMinY = Math.min(globalMinY, transformed[1]);
                        globalMinZ = Math.min(globalMinZ, transformed[2]);

                        globalMaxX = Math.max(globalMaxX, transformed[0]);
                        globalMaxY = Math.max(globalMaxY, transformed[1]);
                        globalMaxZ = Math.max(globalMaxZ, transformed[2]);
                    });
                }
            }
            ...
            if (node.children) {
                node.children.forEach(childIndex => processNode(childIndex, worldMatrix));
            }
        };

        // Commencer par les nœuds racine de la scène
        if (this.gltfData.scenes && this.gltfData.scenes.length > 0) {
            const scene = this.gltfData.scenes[this.gltfData.scene || 0];
            const identityMatrix = mat4.create();
            scene.nodes.forEach(nodeIndex => processNode(nodeIndex, identityMatrix));
        }

        // Calculer les dimensions de base (avec les transformations GLTF)
        const baseWidth = globalMaxX - globalMinX;
        const baseHeight = globalMaxY - globalMinY;
        const baseDepth = globalMaxZ - globalMinZ;

        // Appliquer l'échelle du Model (définie par l'utilisateur)
        const width = baseWidth * modelScaleX;
        const height = baseHeight * modelScaleY;
        const depth = baseDepth * modelScaleZ;

        return {
            min: { x: globalMinX * modelScaleX, y: globalMinY * modelScaleY, z: globalMinZ * modelScaleZ },
            max: { x: globalMaxX * modelScaleX, y: globalMaxY * modelScaleY, z: globalMaxZ * modelScaleZ },
            width: width,
            height: height,
            depth: depth,
            center: {
                x: (globalMinX + globalMaxX) / 2 * modelScaleX,
                y: (globalMinY + globalMaxY) / 2 * modelScaleY,
                z: (globalMinZ + globalMaxZ) / 2 * modelScaleZ
            },
            halfExtents: {
                x: width / 2,
                y: height / 2,
