...
export class Model extends Mesh {
    ...
    setupCollider = (world, modelType) => {
        this.dimensions = this.getOverallDimensions();
        const { halfExtents } = this.dimensions;
    ...
    // TODO: create a updateScale method for the model AND ANOTHER ONE for the collider
    // for the model just update this.scale
    // for the collider update the colliderDesc like here withour updating this.scale
    // create a prop colliderScale ? 
    updateScale(world, value) {
        this.scale = value;
        world.removeCollider(this.collider, true);
        const colliderDesc = RAPIER.ColliderDesc.cuboid(this.scale.x * this.dimensions.halfExtents.x, this.scale.y * this.dimensions.halfExtents.y, this.scale.z * this.dimensions.halfExtents.z)
            .setRestitution(0.5)
            .setFriction(0.7);
        this.collider = world.createCollider(colliderDesc, this.rigidBody);
        this.setColliderUserData(this.collider);
    ...
    updateBodyType(world, value) {
        if (this.rigidBody) {
            world.removeRigidBody(this.rigidBody);
            this.rigidBody = undefined;
            this.collider = undefined;
        }
        this.setupCollider(world, value);
    }
    ...
    getParams() {
        return {    
            ...
            colliderType: { type: 'select', options: ['dynamic', 'fixed'], onChange: this.updateBodyType.bind(this) },
        }
    }
